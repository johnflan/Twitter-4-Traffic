...
class Api(object):
  ...
  def GetSearch(self,
    ...
    Args:
      ...
      geocode:
        ...
        [Optional]
      until:
        Returns tweets generated before the given date. Date should be strings
      ...
      result_type:
        specify 'mixed','recent' or 'popular'. Default is mixed
      lang:
    ...
    for x in data['results']:
      ...
      if query_users:
        ...
        temp.user = User(name=x['from_user_name'],screen_name=x['from_user'], profile_image_url=x['profile_image_url'])
  ...
  def GetFollowers(self, page=None):
    ...
    if not self._oauth_consumer:
      raise TwitterError("twitter.Api instance must be authenticated")
    url = '%s/statuses/followers/ids.json' % self.base_url
    parameters = {}
    if page:
      parameters['page'] = page
    json = self._FetchUrl(url, parameters=parameters)
  ...
  def UsersSearch(self, q=None, page=None, per_page=None, include_entites=None):
    ...
    returned by this API request. Note that unlike GET search, this method does
    not support any operators.
    ...
    Args:
      ...
      include_entities
        ...
        While entities are opt-in on timelines at present, they will be made a
        default component of output in the future. See Tweet Entities for more
        detail on entities.
        [Optional]

    Returns:
      A list of twitter.User objects from the search stream
    '''
    ...
    if not q is None:
      ...
      raise TwitterError("The query term must be specified.")
    if not page is None:
        parameters['page'] = page
    if not per_page is None:
        parameters['per_page'] = per_page
    if not include_entites is None:
        parameters['include_entites'] = include_entites
    json = self._FetchUrl(url, parameters=parameters)
    data = self._ParseAndCheckTwitter(json)
    return [User.NewFromJsonDict(u) for u in data]
